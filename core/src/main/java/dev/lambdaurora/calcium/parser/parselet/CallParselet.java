...
import dev.lambdaurora.calcium.expression.FunctionCallExpression;
...
public class CallParselet implements InfixParselet {
    ...
    public Expression parse(Parser parser, Expression left, Token token) throws ParseException {
        if (!(left instanceof IdentifierExpression idExpr))
            throw new ParseException("Could not parse function call because there is no valid name.", token.offset());
        var id = idExpr.id();
