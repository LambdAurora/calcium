...
import dev.lambdaurora.calcium.expression.AbsoluteExpression;
...
/**
 * Parses an absolute value expression like {@code |a|}.
 */
public class AbsoluteParselet implements PrefixParselet {
    @Override
    public Expression parse(Parser parser, Token token) throws ParseException {
        var expression = parser.parseExpression();
        parser.expect(TokenType.PIPE);
        return new AbsoluteExpression(expression);
    }
}
